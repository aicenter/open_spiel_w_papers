...
namespace open_spiel
{
  namespace liars_dice
  {
    ...
    namespace
    {
      ...
      constexpr int kDefaultdice_sides = 6; // Number of sides on the dice.
      ...
      const GameType kGameType{
          ...
          {{"players", GameParameter(kDefaultPlayers)},
           ...
           {"dice_sides", GameParameter(kDefaultdice_sides)},
    ...
    // Note: This class needs to be declared as a friend in LiarsDiceState:
    // Add this line to the LiarsDiceState class declaration:
    // friend class LiarsDiceObserver;

    class LiarsDiceObserver : public Observer
    {
    public:
      explicit LiarsDiceObserver(IIGObservationType iig_obs_type)
      ...
      {
        const LiarsDiceState &state =
            open_spiel::down_cast<const LiarsDiceState &>(observed_state);
        const LiarsDiceGame &game =
            open_spiel::down_cast<const LiarsDiceGame &>(*state.GetGame());
        SPIEL_CHECK_GE(player, 0);
        SPIEL_CHECK_LT(player, game.NumPlayers());

        // List all predicates.
        const bool imp_info = true; // Liar's dice is always imperfect info
        ...
        // Conditionally write each field - following Goofspiel pattern
        if (imp_info && priv_one)
          WritePlayerNumber(game, state, player, allocator);
        if (imp_info && priv_one)
          WritePlayerDice(game, state, player, allocator);
        if (imp_info && pub_info && perf_rec)
          WriteBidSequence(game, state, allocator);
        if (imp_info && pub_info && !perf_rec)
          WriteRecentBids(game, state, allocator);
      }
                             ...
                             int player) const override
      {
        ...
        if (imp_info && priv_one && perf_rec)
        { // InformationState
          StringPlayerDice(game, state, player, &result);
          StringBidSequence(game, state, &result);
          return result;
        }
        if (imp_info && priv_one && !perf_rec)
        { // Observation
          StringPlayerDice(game, state, player, &result);
          StringRecentBids(game, state, &result);
          StringRemainingDice(game, state, &result);
          return result;
        }
    ...
    private:
      ...
      void WritePlayerNumber(const LiarsDiceGame &game, const LiarsDiceState &state,
                             int player, Allocator *allocator) const
      {
        auto out = allocator->Get("player_number", {game.NumPlayers()});
        out.at(player) = 1.0;
      }

      // Bit vector of observing player's dice.
      void WritePlayerDice(const LiarsDiceGame &game, const LiarsDiceState &state,
                           int player, Allocator *allocator) const
      {
        auto out = allocator->Get("player_dice",
                                  {game.max_dice_per_player() * game.dice_sides()});
        int my_num_dice = state.num_dice_[player];
        for (int d = 0; d < my_num_dice; d++)
        {
          int outcome = state.dice_outcomes_[player][d];
          if (outcome != kInvalidOutcome)
          {
            out.at(d * game.dice_sides() + (outcome - 1)) = 1.0;
          }
        }
      }

      // Remaining dice count for all players.
      void WriteRemainingDiceCount(const LiarsDiceGame &game, const LiarsDiceState &state,
                                   Allocator *allocator) const
      {
        auto out = allocator->Get("remaining_dice", {game.NumPlayers()});
        for (int p = 0; p < game.NumPlayers(); ++p)
        {
          out.at(p) = static_cast<float>(state.num_dice_[p]);
        }
      }

      // Full bid sequence (for perfect recall).
      void WriteBidSequence(const LiarsDiceGame &game, const LiarsDiceState &state,
                            Allocator *allocator) const
      {
        auto out = allocator->Get("bid_sequence",
                                  {game.total_num_dice() * game.dice_sides() + 1});
        for (int b = 0; b < state.bidseq_.size(); ++b)
        {
          out.at(state.bidseq_[b]) = 1.0;
        }
      }

      // Recent bids only (for imperfect recall/observation).
      void WriteRecentBids(const LiarsDiceGame &game, const LiarsDiceState &state,
                           Allocator *allocator) const
      {
        auto out = allocator->Get("recent_bids",
                                  {game.total_num_dice() * game.dice_sides() + 1});
        int size_bid = state.bidseq_.size();
        int bid_offset = std::max(0, size_bid - game.NumPlayers());
      ...
      void StringPlayerDice(const LiarsDiceGame &game, const LiarsDiceState &state,
                            int player, std::string *result) const
      {
        absl::StrAppend(result, absl::StrJoin(state.dice_outcomes_[player], ""));
      }

      void StringBidSequence(const LiarsDiceGame &game, const LiarsDiceState &state,
                             std::string *result) const
      {
        int liar_value = game.total_num_dice() * game.dice_sides();
        for (int b = 0; b < state.bidseq_.size(); ++b)
        {
          if (state.bidseq_[b] == liar_value)
          {
            absl::StrAppend(result, " Liar");
          }
          else
          {
            const std::pair<int, int> bid = state.UnrankBid(state.bidseq_[b]);
            absl::StrAppend(result, " ", bid.first, "-", bid.second);
          }
        }
      }

      void StringRecentBids(const LiarsDiceGame &game, const LiarsDiceState &state,
                            std::string *result) const
      {
        absl::StrAppend(result, "Recent bids: ");
        int liar_value = game.total_num_dice() * game.dice_sides();
        int size_bid = state.bidseq_.size();
        int bid_offset = std::max(0, size_bid - game.NumPlayers());
      ...
      void StringRemainingDice(const LiarsDiceGame &game, const LiarsDiceState &state,
                               std::string *result) const
      {
        absl::StrAppend(result, "Remaining dice: ");
        for (int p = 0; p < game.NumPlayers(); ++p)
        {
          absl::StrAppend(result, "P", p, ":", state.num_dice_[p], " ");
        }
        absl::StrAppend(result, " ");
      }

      void StringIsTerminal(const LiarsDiceState &state,
                            std::string *result) const
      {
    ...
    std::shared_ptr<Observer> LiarsDiceGame::MakeObserver(
        absl::optional<IIGObservationType> iig_obs_type,
        const GameParameters &params) const
    {
      if (!params.empty())
        SpielFatalError("Observation params not supported");
      return std::make_shared<LiarsDiceObserver>(
